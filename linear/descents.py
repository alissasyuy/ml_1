...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
#         index = np.random.choice(list(range(x.shape[0])), self.batch_size)
#         index = np.random.randint(0, x.shape[0], self.batch_size)
#         return super().calc_gradient(x[index], np.array(y)[index])
        index = np.random.randint(0, x.shape[0], self.batch_size)
        return super().calc_gradient(x[index, :],  np.array(y)[index])
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        m_i = self.m / (1 - self.beta_1 ** self.iteration)
        v_i = self.v / (1 - self.beta_2 ** self.iteration)

        self.w = self.w - eta / (np.sqrt(v_i) + self.eps) * m_i
        return -eta / (np.sqrt(v_i) + self.eps) * m_i
